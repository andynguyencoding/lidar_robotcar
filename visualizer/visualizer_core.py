...
def calculate_scale_factor(data_manager, sample_size=10):
    ...
    try:
        # Sample some data points to determine scale
        sample_data = []
        total_frames = len(data_manager.data)
        sample_indices = np.linspace(0, total_frames-1, min(sample_size, total_frames), dtype=int)
        
        for idx in sample_indices:
            if idx < len(data_manager.data):
                frame_data = data_manager.data[idx]
                if isinstance(frame_data, list) and len(frame_data) >= 2:
                    x, y = frame_data[0], frame_data[1]
                    distance = math.sqrt(x*x + y*y)
                    if distance > 0:  # Only include valid distances
                        sample_data.append(distance)
        
        if not sample_data:
            return 1.0  # Default scale factor
            
        # Calculate scale factor based on maximum distance
        max_distance = max(sample_data)
        if max_distance > 0:
            # Scale to fit within reasonable canvas size (assuming 680px canvas)
            canvas_size = 680
            scale_factor = (canvas_size * 0.8) / max_distance  # Use 80% of canvas
            return max(0.1, min(10.0, scale_factor))  # Clamp between 0.1 and 10
